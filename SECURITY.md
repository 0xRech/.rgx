...
RGX `v0.5.0-alpha1` is experimental, unaudited software currently developed on the `test` branch. The latest public release on `main` remains `v0.4.0-alpha.2` until the v0.5 work is intentionally promoted.
...
The archive key is then wrapped independently for each X25519 recipient. The current experimental `RGXR` v2 envelope supports up to 64 recipient slots. An optional password-fallback slot can wrap the same archive key using Argon2id + XChaCha20-Poly1305.

The BLAKE3 digest of the complete recipient envelope is included in the authenticated `RGXK` stream header. As a result, recipient-slot and password-slot metadata are bound to the encrypted payload and envelope modification causes verification to fail.
...
A v0.5 public identity contains:
...
The default `rgx keygen` mode stores the private key material in an RGX key file so automatic recipient unlock can happen without an additional prompt. On Unix the file is created with mode `0600`.
...
Verification recomputes the archive digest and checks the Ed25519 signature using the trusted RGX public key supplied by the verifier.
...
The `test` branch uses cross-platform unit/integration tests, Clippy/format checks, dependency audits, static Linux builds, and fuzz smoke tests. Current fuzz targets include both the plain archive parser and the `RGXR` recipient-envelope parser.
